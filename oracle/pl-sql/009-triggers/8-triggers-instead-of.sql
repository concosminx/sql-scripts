...
/*
  instead of trigger
  - used with views
  - enabling DML on complex views (un-updatable)
  - bypass the check constraint on view
  - before and after timing not available
*/

/*view*/
CREATE OR REPLACE VIEW VW_EMP_DETAILS AS
  SELECT UPPER(DEPARTMENT_NAME) DNAME,
    MIN(SALARY) MIN_SAL,
    MAX(SALARY) MAX_SAL
  FROM EMPLOYEES_COPY
...
CREATE OR REPLACE TRIGGER EMP_DETAILS_VW_DML INSTEAD OF
  INSERT OR UPDATE OR DELETE ON VW_EMP_DETAILS FOR EACH ROW 
DECLARE 
