...
/*
    associative arrays
    a.k.a index by tables
    
    - the key cat have a string
    - keys done not need to be sequential
    - do not initialise associtive arrays
    - associative arrays are indexes (b-tree if u are using varchar2)
*/
DECLARE
    TYPE e_list IS
...
/*iteration example for associative arrays*/
DECLARE
    TYPE e_list IS
        TABLE OF employees.first_name%TYPE INDEX BY PLS_INTEGER;
    emps  e_list;
...
/*arrays with records type (db) */
DECLARE
    TYPE e_list IS
        TABLE OF employees%rowtype INDEX BY employees.email%TYPE;
    emps  e_list;
    idx   employees.email%TYPE;
BEGIN
    FOR x IN 100..110 LOOP
...
DECLARE
    ...
    TYPE e_list IS
        TABLE OF e_type INDEX BY employees.email%TYPE;
    emps  e_list;
    idx   employees.email%TYPE;
BEGIN
    FOR x IN 100..110 LOOP
...
/* inserting with associative arrays */
...
BEGIN
    ...
    WHILE idx IS NOT NULL LOOP
        ...
        dbms_output.put_line('The employee ' || emps(idx).first_name || ' is inserted to the history table');
        idx := emps.next(idx);
    END LOOP;
END;
